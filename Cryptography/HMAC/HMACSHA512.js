...
// Main HMAC-SHA512
// Input: secret (string), message (string)
// Output: HMAC-SHA512 as hex string
function hmacsha512(secret, message) {
    ...
    // 1. If key is too long, hash it first
    if (key.length > blockSize) {
        key = sha512Compute(key);
    }

    // 2. Pad key to block size
    var k0 = new Array(blockSize);
    for (var i = 0; i < blockSize; i++) {
        k0[i] = (i < key.length) ? key[i] : 0;
    }

    // 3. Create inner and outer pads
    ...
    // 4. Perform HMAC: Hash(opad + Hash(ipad + message))
    var innerParams = ipad.concat(msg);
    var innerHash = sha512Compute(innerParams);

    var outerParams = opad.concat(innerHash);
    var outerHash = sha512Compute(outerParams);
